...
const DropDown = React.forwardRef(
  (
    ...
    const openUpward = openOptions === "upward";
    ...
    return (
      <div className="relative w-full min-w-auto max-w-full" ref={ref}>
        ...
        {isOpen && (
          <ul
            className={`max-h-48 absolute ${
              openUpward ? "-top-52" : "bottom-0"
            } right-1/2 translate-x-1/2 z-50 w-full min-w-56 mt-1 bg-secondary-bg dark:bg-secondary-bg-dark rounded-md shadow-lg text-primary-text dark:text-primary-text-dark border border-border dark:border-border-dark overflow-y-auto`}
          >
            {options?.map((option) => (
              <li
                key={option}
